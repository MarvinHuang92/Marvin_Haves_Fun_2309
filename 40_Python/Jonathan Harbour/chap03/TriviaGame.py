...
class Trivia(object):
    ...
    def show_question(self):
        ...
        if self.asking_restart:
            print_text(font1, 80, 410, "Would you like to restart the game?", green)
            print_text(font1, 100, 450, "Press Y-yes, restart    N-no, exit", green)

    def handle_input(self,number):
        if not self.scored and not self.failed and not self.asking_restart:
    ...
    def restart_input(self,choice):
        if not self.restart and not self.exit and self.asking_restart:
            if choice == 1:
                self.restart = True
                self.asking_restart = False
            elif choice == 0:
    ...
    for event in pygame.event.get():
        if event.type == QUIT:
            ...
            if event.key == pygame.K_ESCAPE:
                ...
                trivia.restart_input(1)
            elif event.key == pygame.K_n:
                trivia.restart_input(0)
            elif event.key == pygame.K_RETURN:
                if trivia.current >= trivia.total - 6:
                    trivia.asking_restart = True
                    trivia.ask_restart()
                else:
                    trivia.asking_restart = False
