...
# 额外的方法
    ...
    def collision(self, name, location, direction, size, id):
        ...
        for entity in self.entities2.values():
            ...
            if id != entity.id:
                # 是否有碰撞体积
                if (name == "tank" and entity.collision_to_tank) or (name == "canon" and entity.collision_to_canon):
                    # 获得自身到对方的距离向量
...
class Terran(GameEntity):  # 地形对象，不可以动，中立阵营
    def __init__(self, world, name, image):
        self.name = name
        GameEntity.__init__(self, world, "terran", image)
...
class Forrest(Terran):  # 树，没有碰撞体积
    def __init__(self, world, image):
        ...
        self.collision_tank = False
        self.collision_canon = False
...
class TankStateGoing(State):  # 包括完整的中途动作、转移条件、进出动作
    ...
    def check_conditions(self):  # 撞到什么东西，进入stopping状态
        goal = self.tank.world.collision("tank", self.tank.location, self.tank.direction, TK, self.tank.id)
        if goal is not None:
            return "stopping"
        return None
...
def run():
    ...
    pygame.display.set_caption('Tank Battle v0.2')  # pygame下的窗口标题，和python主体的设置方法不一样
    world = World()
    # 这里定义的小写world变量名是一个实例，用来在后面传递给其他类。当然，World类只有这一个实例
    w, h = BATTLEFIELD
    ...
    while True:
        ...
        for entity in world.entities2.values():
            if entity.name == "tank":  # 如果是坦克，就停止移动
        ...
        text_surface1 = font.render('Time: %d' % TIME, True, (255, 255, 255))
        text_surface2 = font.render('Enemies: %d' % ENEMY_NUMBER, True, (255, 255, 255))
        ...
        if ENEMY_NUMBER == 0:
            screen.blit(text_surface3, (BATTLEFIELD[0] + 35, 95))
            pressed_keys = pygame.key.get_pressed()
            if pressed_keys[K_SPACE]:
                exit()
        if GAMEOVER:
            screen.blit(text_surface4, (BATTLEFIELD[0] + 5, 95))
            pressed_keys = pygame.key.get_pressed()
            if pressed_keys[K_SPACE]:
                exit()
