...
while True:
    ...
    if keys[K_ESCAPE]:
        sys.exit()
    elif keys[K_SPACE] or autopilot:
    ...
    # 自动导航执行逻辑（精确等级控制）
    if abs(delta_y) > d_delta_course:
        # 快速的速度控制
        if drop_vel > d_v_course:  # 下降太快时，开启机动
            autopilot = True
        elif drop_vel + d_v_course < 0:
            autopilot = False  # 上升太快时，关闭机动
        else:  # 速度合适时
        # 距离控制
            if abs(delta_y) < d_delta_course:
                autopilot = False
            elif delta_y > d_delta_course:  # 高度太低时，上升
                autopilot = True
            else:
                autopilot = False  # 确保其他状态的flag是正确的

    # 同上一层逻辑相同，全都换成fine
    else:  # 精细控制
        if drop_vel > d_v_fine:  # 下降太快时，开启机动
            autopilot = True
        elif drop_vel + d_v_fine < 0:
            autopilot = False  # 上升太快时，关闭机动
        else:  # 速度合适时，对距离控制
            if abs(delta_y) < d_delta_fine:
                autopilot = False
            elif delta_y > d_delta_fine:  # 高度太低时，上升
                autopilot = True
            else:
                autopilot = False  # 确保其他状态的flag是正确的
    ...
    print_text(font, 5, 5, "Score:")
    print_text(font, 155, 5, str(score))
    print_text(font, 5, 40, "Next_Gap:")
    print_text(font, 155, 40, str(int(next_y)))
    print_text(font, 5, 75, "Current_Y:")
    print_text(font, 155, 75, str(int(current_y)))
    print_text(font, 5, 110, "Delta_Y:")
    print_text(font, 155, 110, str(int(delta_y)))
    print_text(font, 5, 145, "Speed_Y:")
    print_text(font, 155, 145, str("{:.1f}".format(drop_vel)))
    if autopilot:
        print_text(font, 5, 215, "<AUTOPILOT: ON >")
    else:
        print_text(font, 5, 215, "<AUTOPILOT: OFF>")

    if game_over:
        print_text(font, 5, 180, "GAME OVER")
