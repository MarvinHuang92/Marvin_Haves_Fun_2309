...
try:
    ...
    log.write("Can not find DWG File List!\n=======================\n")
...
for dwg_name in file_name_list:
    ...
    try:
        ...
        try:
            ...
            log.write("Invalid File Name: %s" %(dwg_name))
            working = False
        ...
        if 0 < part_no < 1000:
            try:
                log.write("Open file: %s\n"%(dwg_name))
                acad.ActiveDocument.Application.Documents.Open("%s%s" %(DWG_File_path, dwg_name))
            except:
                log.write("Can not open this file\n")
                working = False
        else:
            log.write("Invalid Part Number: %s\n" %(dwg_name))
            working = False


        # 如果文件名合法
        if working:
            log.write("Start working on Part #%s\n"%(DET))
            ...
            try:                
                ...
                log.write("Work finished on Part #%s\n\n"%(DET))

            except:
                log.write("Work NOT finished on Part #%s\n\n"%(DET))
            ...
            log.write("Part #%s Doesn't Exist!\n" %(DET))
...
log.write('ErrorNumber:')
for i in ErrorNumber:
        log.write(i)
log.close()
log.write('More Information Can be Found in Log.txt')
