...
# Billiard 
tabletapete=box(size=(10.75,0.5,13.75+2*maxposx),color=color.green)
...
l4=box(color=color.orange,size=(0.5,1,13+2*maxposx),pos=(-5.25,0.5,0),material=materials.wood)estadio=curve(color=color.orange,radius=0.35,material=materials.wood)
for i in arange(180): 
    theta=i*pi/180
    estadio.append([(maxposx+0.75)*cos(theta),0.5,maxposz+(maxposx+0.75)*sin(theta)])for i in arange(180): 
        theta=i*pi/180 
...
bola.append(sphere(color=color.yellow,radius=0.5,pos=(2,0.75,-5), material=materials.plastic))#for i in arange(4):
